...
def train(model: Model,
    ...
    while True:
        ...
        while loader.has_next():
            ...
            print(f'[TRAIN] Epoch: {epoch} Batch: {iter_info[0]}/{iter_info[1]} Loss: {loss}')
...
def validate(model: Model, loader: DataLoaderIAM) -> Tuple[float, float]:
    ...
    while loader.has_next():
        ...
        for i in range(len(recognized)):
            ...
            if dist != 0:
                print('[VAL][ERR:%d]' % dist, '"' + batch.gt_texts[i] + '"', '->', '"' + recognized[i] + '"')

    # print validation result
    char_error_rate = num_char_err / num_char_total
    word_accuracy = num_word_ok / num_word_total
    print(f'[VAL] Character error rate: {char_error_rate * 100.0}%.\n[VAL] Word accuracy: {word_accuracy * 100.0}%.')
...
def main():
    ...
    if args.mode == 'train':
        ...
        with open(FilePaths.fn_corpus, 'w') as f:
            f.write(' '.join(loader.train_words + loader.validation_words + loader.test_words))
    ...
    # evaluate it on the validation set
        ...
        print('Test NN\n[TEST] running on test set')
        loader = DataLoaderIAM(args.data_dir, args.batch_size, fast=args.fast)
        model = Model(char_list_from_file(), decoder_type, must_restore=True)       
        loader.test_set()   # Switch loader to the test set
        validate(model, loader) # Reuse the same validate() function to compute metrics
...
if __name__ == '__main__':
    ...
    VALIDATE:
    python main.py --mode validate --data_dir ../dataset
